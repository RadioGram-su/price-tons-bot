...
async function ensureSymbolIndex() {
  ...
  const data = await stonFetch("/assets?limit=500");
...
async function resolveAsset(query) {
  ...
  const exact = index.bySymbol.get(text.toUpperCase()) || [];
  if (exact.length === 1) return formatAsset(exact[0]);

  const partial = [];
  for (const [symbol, assets] of index.bySymbol.entries()) {
    if (symbol.includes(text.toUpperCase())) partial.push(...assets.slice(0, 1));
  }
  partial.sort((a, b) => Number(b.popularity_index || 0) - Number(a.popularity_index || 0));

  if (exact.length > 1) {
    return { multiple: exact.slice(0, 8).map(formatAsset) };
  }
  if (partial.length) {
    return { multiple: partial.slice(0, 8).map(formatAsset) };
  }
